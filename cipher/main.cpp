...
int main()
{
    ...
    if (mode == "encrypt") {
        ...
        ofstream user_password_file("/home/stud/user_password_file");
        StringSource(pass, true, new FileSink(user_password_file));
        ofstream key_file("/home/stud/key_file");
        ArraySource(key, sizeof(key), true, new FileSink(key_file));
        ofstream IV_file("/home/stud/iv_file");
        ...
        string user_pass;
        FileSource("/home/stud/user_password_file", true, new StringSink(user_pass));
        if (pass != user_pass) {
        ...
        cin >> input_file_string;
        if(input_file_string == "default")
            input_file_string = "/home/stud/encrypted_text_file";
        ifstream input_check(input_file_string);
        if(input_check.is_open() == 0) {
        ...
        cin >> output_file_string;
        if(output_file_string == "default")
            output_file_string = "/home/stud/decrypted_text_file";
        ifstream output_check(output_file_string);
        if(output_check.is_open() == 0) {
        ...
        byte key[Tiger::DIGESTSIZE];
        FileSource("/home/stud/key_file", true, new ArraySink(key, sizeof(key)));
        byte IV[ AES::BLOCKSIZE ];
        FileSource("/home/stud/iv_file", true, new ArraySink(IV, sizeof(IV)));
